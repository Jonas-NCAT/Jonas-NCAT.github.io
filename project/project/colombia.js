...
$(document).ready(function () {
    // Fetch local time for Bogotá, Colombia
    //This section sets up the function getLocalTime(), which is responsible for fetching the local time in Bogotá using the TimeZoneDB API.
    function getLocalTime() {
        const apiKey = "S3ZHD4FMWOHA"; // TimeZoneDB API, stores the API key necessary to authenticate the request to TimeZoneDB (a third-party service that provides time zone data).
        const apiUrl = `https://api.timezonedb.com/v2.1/get-time-zone?key=${apiKey}&format=json&by=zone&zone=America/Bogota`;  // This is the URL to which the GET request is sent, incorporating the API key and the specific time zone (America/Bogota).
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data && data.formatted) {
                    ...
                    $("#local-time").text(`Local time in Bogotá, Colombia: ${formattedTime}`);
                } else {
    ...
    // Fetch weather data for Bogotá, Colombia
    // OpenWeatherMap API key, stores the API key for the OpenWeatherMap service.
    //City is set to "Bogota", specifying the city for which weather data is being requested.
    ...
    const apiKey = "a1cabc272d1b259442b247df2a49742c";  
    const city = "Bogota"; 
