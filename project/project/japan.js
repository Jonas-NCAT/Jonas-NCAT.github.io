...
$(document).ready(function () {
    // Fetch local time for Tokyo, Japan
    //This section sets up the function getLocalTime(), which is responsible for fetching the local time in Tokyo using the TimeZoneDB API.
    function getLocalTime() {
        ...
        const apiUrl = `https://api.timezonedb.com/v2.1/get-time-zone?key=${apiKey}&format=json&by=zone&zone=Asia/Tokyo`; // This is the URL to which the GET request is sent, incorporating the API key and the specific time zone (Asia/Tokyo).
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data && data.formatted) {
                    ...
                    $("#local-time").text(`Local time in Tokyo, Japan: ${formattedTime}`);
                } else {
    ...
    // Fetch weather data for Tokyo, Japan
    // OpenWeatherMap API key, stores the API key for the OpenWeatherMap service.
    //City is set to "Tokyo", specifying the city for which weather data is being requested.
