...
$(document).ready(function () {
    // Fetch local time for Accra, Ghana
    //This section sets up the function getLocalTime(), which is responsible for fetching the local time in Accra using the TimeZoneDB API.
    function getLocalTime() {
        ...
        const apiUrl = `https://api.timezonedb.com/v2.1/get-time-zone?key=${apiKey}&format=json&by=zone&zone=Africa/Accra`; // This is the URL to which the GET request is sent, incorporating the API key and the specific time zone (Africa/Accra).
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data && data.formatted) {
                    ...
                    $("#local-time").text(`Local time in Accra, Ghana: ${formattedTime}`);
                } else {
    ...
    // Fetch weather data for Accra, Ghana
    // OpenWeatherMap API key, stores the API key for the OpenWeatherMap service.
    //City is set to "Accra", specifying the city for which weather data is being requested.
